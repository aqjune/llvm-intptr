...
bool llvm::isSafeToPropagatePtrEquality(Value *Op0, Value *Op1,
  ...
  SmallVector<Value *, 4> Op0Bases, Op1Bases;
	GetUnderlyingObjects(Op0, Op0Bases, DL, nullptr, 6, false);
	GetUnderlyingObjects(Op1, Op1Bases, DL, nullptr, 6, false);
  ...
  if (GEP0 && GEP1 && GEP0->isInBounds() && GEP1->isInBounds()) {
    if (GetUnderlyingObject(Op0, DL, 6, true) ==
        GetUnderlyingObject(Op1, DL, 6, true))
      return true;
    else {
      SmallVector<Value *, 4> Op0Bases2, Op1Bases2;
      GetUnderlyingObjects(Op0, Op0Bases2, DL, nullptr, 6, true);
      GetUnderlyingObjects(Op1, Op1Bases2, DL, nullptr, 6, true);
